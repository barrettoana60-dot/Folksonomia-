...
body {
    background: linear-gradient(130deg, #bcbcbc 0%, #d7d7d7 32%, #f3f3f3 56%, #cfcfcf 100%);
}
.stApp {
    color: var(--text-1);
    background:
        radial-gradient(circle at 14% 18%, rgba(255,255,255,0.54) 0%, rgba(255,255,255,0.05) 32%, transparent 60%),
        radial-gradient(circle at 82% 14%, rgba(255,255,255,0.42) 0%, rgba(255,255,255,0.04) 24%, transparent 54%),
        radial-gradient(circle at 80% 80%, rgba(255,255,255,0.36) 0%, rgba(255,255,255,0.04) 20%, transparent 46%),
        linear-gradient(135deg, #b7b7b7 0%, #d7d7d7 30%, #f1f1f1 55%, #cccccc 100%);
...
.topbar {
    ...
    padding: 1rem 1.2rem; border-radius: 24px;
    background: rgba(255,255,255,0.20); border: 1px solid rgba(255,255,255,0.50);
    backdrop-filter: blur(26px) saturate(160%); -webkit-backdrop-filter: blur(26px) saturate(160%);
    box-shadow: 0 16px 40px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.45);
}
.brand-title { font-family: "Times New Roman", Georgia, serif; font-size: 2rem; font-weight: 700; letter-spacing: -0.05em; text-transform: lowercase; color: #1f1f1f; }
.brand-subtitle { font-size: 0.76rem; color: #666666; letter-spacing: 0.18em; text-transform: uppercase; }
.status-chip {
    display: inline-flex; align-items: center; min-height: 36px; padding: 0.6rem 1rem; border-radius: 999px;
    background: rgba(255,255,255,0.16); border: 1px solid rgba(255,255,255,0.40); color: #414141; font-size: 0.82rem;
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.46);
}
.hero-panel, .panel {
    background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.46);
    border-radius: 28px; backdrop-filter: blur(26px) saturate(160%); -webkit-backdrop-filter: blur(26px) saturate(160%);
    box-shadow: 0 18px 46px rgba(0,0,0,0.10), inset 0 1px 0 rgba(255,255,255,0.46);
}
.hero-panel { padding: 2rem; margin-top: 1rem; }
.hero-grid { display: grid; grid-template-columns: 1.15fr 0.85fr; gap: 1.2rem; }
.hero-kicker { font-size: 0.84rem; text-transform: uppercase; letter-spacing: 0.20em; color: #6f6f6f; }
.hero-title { font-family: "Times New Roman", Georgia, serif; font-size: 4rem; line-height: 0.92; letter-spacing: -0.06em; color: #1f1f1f; }
...
.kpi-box, .metric-card, .story-card, .preview-card, .work-card, .queue-card, .suggestion-card {
    background: rgba(255,255,255,0.16); border: 1px solid rgba(255,255,255,0.38); border-radius: 24px;
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.44), 0 10px 28px rgba(0,0,0,0.06);
}
...
.kpi-value, .metric-number { font-family: "Times New Roman", Georgia, serif; font-size: 2.1rem; color: #202020; letter-spacing: -0.05em; font-weight: 700; margin-top: 0.35rem; }
...
.work-card { padding: 0.75rem; min-height: 520px; }
.work-card img { width: 100%; height: 260px; object-fit: cover; border-radius: 20px; display: block; }
...
.tag-chip, .badge-soft {
    ...
    background: rgba(255,255,255,0.18); border: 1px solid rgba(255,255,255,0.38); color: #363636; font-size: 0.82rem;
}
.soft-line { width: 100%; height: 1px; background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.7) 50%, transparent 100%); margin: 0.8rem 0 1rem 0; }
.stTabs [data-baseweb="tab-list"] { gap: 0.6rem; padding: 0.45rem; border-radius: 18px; background: rgba(255,255,255,0.14); border: 1px solid rgba(255,255,255,0.36); }
.stTabs [data-baseweb="tab"] { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.36); border-radius: 14px; color: #383838; font-weight: 600; }
.stTabs [aria-selected="true"] { background: rgba(255,255,255,0.28) !important; color: #1f1f1f !important; }
.stButton > button, .stDownloadButton > button {
    min-height: 46px; padding: 0.72rem 1.1rem !important; border-radius: 16px !important;
    background: rgba(255,255,255,0.16) !important; border: 1px solid rgba(255,255,255,0.42) !important; color: #2f2f2f !important;
    font-weight: 600 !important; box-shadow: inset 0 1px 0 rgba(255,255,255,0.46), 0 8px 20px rgba(0,0,0,0.06) !important;
}
.stButton > button:hover, .stDownloadButton > button:hover { background: rgba(255,255,255,0.24) !important; }
.stTextInput input, .stTextArea textarea, .stSelectbox div[data-baseweb="select"] > div, .stMultiSelect div[data-baseweb="select"] > div, .stNumberInput input {
    background: rgba(255,255,255,0.17) !important; border: 1px solid rgba(255,255,255,0.42) !important; border-radius: 16px !important; color: #2b2b2b !important;
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.46) !important;
}
.stAlert { background: rgba(255,255,255,0.16) !important; border: 1px solid rgba(255,255,255,0.42) !important; border-radius: 18px !important; }
[data-testid="stDataFrame"] { background: rgba(255,255,255,0.14) !important; border-radius: 18px !important; border: 1px solid rgba(255,255,255,0.36) !important; overflow: hidden; }
...
@media (max-width: 640px) { .hero-title { font-size: 2.8rem; } .hero-microgrid, .metric-strip { grid-template-columns: 1fr; } }
</style>
"""
...
class SemanticLearner:
    ...
    def build_concept_matrix(self) -> None:
        ...
        if not rows or not SKLEARN_AVAILABLE:
    ...
    def train(self) -> None:
        ...
        if (not SKLEARN_AVAILABLE) or len(corpus) < 20 or corpus["label"].nunique() < 2:
            ...
            self.store.save_model_state({
                ...
                "mode": "heuristic" if not SKLEARN_AVAILABLE else "insufficient_data",
            })
            return
    ...
    def suggest_concepts(self, tag_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        if not self.concept_rows:
            return []
        if self.concept_vectorizer is None or self.concept_matrix is None or not SKLEARN_AVAILABLE:
            scored = []
            for row in self.concept_rows:
                score = hybrid_similarity(tag_text, row.get("text", "")) if tag_text else 0.0
                payload = dict(row)
                payload["similarity"] = float(score)
                scored.append(payload)
            scored.sort(key=lambda item: item.get("similarity", 0.0), reverse=True)
            return [row for row in scored[:top_k] if row.get("similarity", 0.0) > 0]
        ...
        if not SKLEARN_AVAILABLE:
            ...
            for term in unique_values:
                ...
                for other in unique_values:
                    if other in used:
                        continue
                    if hybrid_similarity(term, other) >= threshold:
                        current.append(other)
                        used.add(other)
                if len(current) > 1:
                    groups.append(sorted(current))
            groups.sort(key=lambda item: (-len(item), item[0]))
            return groups
...
def graph_to_plot(graph: Any, max_nodes: int = 120) -> Any:
    ...
    if len(g.nodes()) == 0:
        return None if not PLOTLY_AVAILABLE else go.Figure()
...
def intro_flow(store: JsonStore) -> None:
    open_panel("acesso inicial", "perfil rápido para liberar a camada pública e registrar o contexto do participante.")
    with st.form("intro_form"):
        c1, c2 = st.columns(2)
        with c1:
            familiarity = st.selectbox("frequência de visita a museus", ["nunca", "raramente", "ocasionalmente", "frequentemente"])
            documentation = st.selectbox("conhecimento sobre documentação museológica", ["nenhum", "básico", "intermediário", "avançado"])
        with c2:
            understanding = st.text_area("o que você entende por tags aplicadas a acervos", height=180, placeholder="escreva livremente")
        submitted = st.form_submit_button("entrar na interface")
        if submitted:
    ...
    with c1:
        if not tags_df.empty:
            ...
            if PLOTLY_AVAILABLE and px is not None:
                ...
                st.bar_chart(top_tags.set_index("tag")["frequência"])
...
def render_public_explore(store: JsonStore, ml: SemanticLearner) -> None:
    works = store.works()
    tags_df = build_tag_dataframe(store)
    open_panel("explorar obras", "filtros, leitura de contexto, linguagem social e escrita translúcida no mesmo espaço.")
    c1, c2, c3 = st.columns([1.4, 1, 1])
    with c1:
        search = st.text_input("buscar por obra, artista, descrição ou tag", placeholder="procure um eixo semântico")
    with c2:
        artist_filter = st.selectbox("artista", ["todos"] + sorted({w.get("artist", "") for w in works}))
    with c3:
        year_filter = st.selectbox("ano", ["todos"] + sorted({w.get("year", "") for w in works}))
    filtered = []
    for work in works:
        haystack = " ".join([work.get("title", ""), work.get("artist", ""), work.get("year", ""), work.get("description", ""), " ".join(work.get("institutional_tags", []))]).lower()
        tag_text = ""
        if not tags_df.empty:
            related = tags_df[tags_df["work_id"] == work.get("id")]
            if not related.empty:
                tag_text = " ".join(related["tag"].astype(str).tolist()).lower()
        cond_search = not search or search.lower() in haystack or search.lower() in tag_text
        cond_artist = artist_filter == "todos" or work.get("artist") == artist_filter
        cond_year = year_filter == "todos" or work.get("year") == year_filter
        if cond_search and cond_artist and cond_year:
    ...
    if not filtered:
        ...
        for idx, work in enumerate(filtered):
            related_df = tags_df[tags_df["work_id"] == work.get("id")] if not tags_df.empty else pd.DataFrame()
            tag_html = ""
            if not related_df.empty:
                counts = related_df["tag"].value_counts().head(8)
                tag_html = "".join([f"<span class='tag-chip'>{tag} {count}</span>" for tag, count in counts.items()])
            with cols[idx % 3]:
                st.markdown("<div class='work-card'>", unsafe_allow_html=True)
                st.image(work.get("image_url"), use_container_width=True)
                st.markdown(f"<div class='work-card-title'>{work.get('title')}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='work-card-meta'>{work.get('artist')} · {work.get('year')}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='work-card-text'>{first_sentence(work.get('description'))}</div>", unsafe_allow_html=True)
                st.markdown("<div class='work-section-label'>tags institucionais</div>", unsafe_allow_html=True)
                st.markdown("".join([f"<span class='tag-chip'>{tag}</span>" for tag in work.get("institutional_tags", [])]) or "<span class='badge-soft'>sem dados</span>", unsafe_allow_html=True)
                st.markdown("<div class='work-section-label'>camada social</div>", unsafe_allow_html=True)
                st.markdown(tag_html or "<span class='badge-soft'>sem participação ainda</span>", unsafe_allow_html=True)
                if st.button("abrir painel da obra", key=f"open-work-{work.get('id')}", use_container_width=True):
                    st.session_state["selected_work_id"] = work.get("id")
                st.markdown("</div>", unsafe_allow_html=True)
    close_panel()
    if st.session_state.get("selected_work_id"):
        render_work_detail(store, ml, st.session_state["selected_work_id"])
    ...
    if not related_df.empty:
        ...
        with c2:
            ...
            if PLOTLY_AVAILABLE and px is not None:
                ...
                st.bar_chart(entity_counts.set_index("categoria")["frequência"])
...
def render_public_contribute(store: JsonStore, ml: SemanticLearner) -> None:
    ...
    open_panel("contribuir", "submissão de tags livres com classificação preditiva, sugestões de conceitos e comentário opcional.")
    if not user:
        st.info("conclua o acesso inicial antes de contribuir.")
        close_panel()
        return
    works = store.works()
    work_map = {f"{w.get('title')} · {w.get('artist')} · {w.get('year')}": w for w in works}
    with st.form("contribute_form"):
        work_choice = st.selectbox("obra", list(work_map.keys()))
        tag_value = st.text_input("tag livre", placeholder="escreva a palavra ou expressão que deseja associar")
        comment_value = st.text_area("comentário interpretativo", height=120, placeholder="justifique a marcação, descreva forma, material, tema ou contexto")
        submitted = st.form_submit_button("registrar tag")
        if submitted:
            if not tag_value.strip():
                st.warning("escreva uma tag antes de registrar.")
            else:
                work = work_map[work_choice]
                payload = store.submit_tag(work.get("id"), user.get("id"), tag_value, comment_value, ml)
                run_automation_engine(store, ml)
                st.session_state["last_submitted_tag_id"] = payload.get("id")
                st.success("tag registrada com camada semântica inicial.")
                st.rerun()
    last_id = st.session_state.get("last_submitted_tag_id")
    if last_id:
        tag_row = next((t for t in store.tags() if t.get("id") == last_id), None)
        if tag_row:
            features = ml.term_features(tag_row.get("tag", ""))
            st.markdown(f"<div class='suggestion-card'><div class='suggestion-title'>leitura inicial da última tag registrada</div><div class='suggestion-meta'>normalização {features.get('normalized')}<br>categoria prevista {features.get('entity')} · confiança {features.get('confidence', 0.0):.2f}</div></div>", unsafe_allow_html=True)
...
def render_public_history(store: JsonStore) -> None:
    ...
    if PLOTLY_AVAILABLE and px is not None:
        ...
        st.bar_chart(top.set_index("tag")["frequência"])
...
def render_admin_graph(store: JsonStore) -> None:
    ...
    graph_view = graph_to_plot(graph, max_nodes=140)
    if PLOTLY_AVAILABLE and graph_view is not None:
        st.plotly_chart(graph_view, use_container_width=True)
    elif isinstance(graph_view, pd.DataFrame) and not graph_view.empty:
        st.info("plotly não está instalado neste ambiente. exibindo a malha do grafo em tabela.")
        st.dataframe(graph_view, use_container_width=True, hide_index=True)
    else:
        st.info("não há dados suficientes para desenhar o grafo.")
...
def render_admin_data(store: JsonStore, ml: SemanticLearner) -> None:
    open_panel("obras e exportação", "cadastro de obras, importação simples e download da base em csv.")
    t1, t2, t3 = st.tabs(["obras", "nova obra", "exportação"])
    with t1:
        works_df = to_dataframe(store.works())
...
def main() -> None:
    ...
    if not st.session_state.get("intro_complete", False) and store.settings().get("public_intro_enabled", True):
        intro_flow(store)
    public_tabs = st.tabs(["visão geral", "explorar", "contribuir", "descoberta semântica", "meu histórico", "administração"])
    with public_tabs[0]:
        render_public_overview(store, ml)
    with public_tabs[1]:
        render_public_explore(store, ml)
    with public_tabs[2]:
        render_public_contribute(store, ml)
    with public_tabs[3]:
