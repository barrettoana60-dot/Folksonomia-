...
def pie_chart_from_series(series, title="Distribuição"):
    ...
    for _, row in df.iterrows():
        pct = float(row["Percentual"]) / 100
        st.markdown(
            f"""
            <div style="margin-bottom:10px;padding:12px;border-radius:12px;
                        background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);">
                <div style="display:flex;justify-content:space-between;font-weight:600;gap:10px;">
                    <span>{row["Categoria"]}</span>
                    <span>{row["Valor"]} ({row["Percentual"]}%)</span>
                </div>
                <div style="margin-top:8px;background:rgba(255,255,255,.10);height:10px;border-radius:999px;overflow:hidden;">
                    <div style="width:{max(2, pct*100)}%;height:100%;background:#7dd3fc;"></div>
                </div>
            </div>
            """,
            unsafe_allow_html=True
...
def line_chart_from_df(df, xcol, ycol, title=""):
    ...
    if title:
        st.markdown(f"#### {title}")
    chart_df = df[[xcol, ycol]].copy()
    chart_df = chart_df.set_index(xcol)
...
def tab_overview():
    ...
    if not tdf.empty:
        ...
        c3, c4 = st.columns(2)
        with c3:
            pie_chart_from_series(
                tdf["obra_id"].map(lambda x: f"Obra {x}").value_counts(),
                "Distribuição de Tags por Obra"
            )
        with c4:
            pie_chart_from_series(
                tdf["tag"].value_counts().head(10),
...
def tab_tags():
    ...
    with t3:
        ...
        with c2:
            monthly = tf.groupby("mes").size()
            bar_chart_from_series(monthly, "Tags por mês")
