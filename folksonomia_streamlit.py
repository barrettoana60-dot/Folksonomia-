...
def css() -> str:
    ...
    card = "rgba(255,255,255,0.78)" if not contrast else "rgba(255,255,255,0.96)"
    border = "rgba(120,140,170,0.25)" if not contrast else "rgba(0,0,0,0.32)"
    shadow = "0 12px 40px rgba(30,41,59,0.08)"
    button_bg = "rgba(255,255,255,0.50)" if not contrast else "rgba(255,255,255,0.92)"
    accent = "#325bff" if not contrast else "#000000"
    return f"""
    <style>
    * {{
    ...
    .hero {{
        ...
        border-radius: 28px;
        background: linear-gradient(135deg, rgba(255,255,255,0.85), rgba(240,245,255,0.82));
    ...
    .chip {{
        ...
        border-radius: 999px;
        margin: 0.16rem;
        background: rgba(50,91,255,0.12);
        border: 1px solid rgba(50,91,255,0.25);
        color: {accent};
        font-size: 0.93rem;
    }}
    .work-card {{
    ...
    .ledger-row {{
        ...
        background: rgba(50,91,255,0.05);
    ...
    .stButton > button:hover {{
        border-color: rgba(50,91,255,0.42) !important;
    ...
    .stTabs [aria-selected="true"] {{
        background: rgba(50,91,255,0.12) !important;
    }}
    .small-note {{
        font-size: 0.88rem;
        opacity: 0.78;
    ...
    components.html(
        ...
        <div>
            ...
            ' style="
                ...
                font-size:1rem;">{html.escape(label)}</button>
        </div>
        """,
        height=58,
        key=key,
...
def access_panel() -> None:
    ...
    st.markdown(
        ...
        <div class="side-access">
            ...
            <div class="mini-help">Painel lateral para contraste, fonte e fundo branco em todo o sistema.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def public_area() -> None:
    ...
    with c1:
        filtro = st.text_input("Filtrar obras por título, artista ou período")
    with c2:
    ...
    if filtro.strip():
        ...
        filtered = [
            o for o in obras
            if nf in normalize(o["titulo"]) or nf in normalize(o["artista"]) or nf in normalize(o.get("periodo", ""))
        ]
    ...
    for idx, obra in enumerate(filtered):
        with cols[idx % 2]:
            ...
            with st.expander("Ler texto curatorial e semântico"):
                st.write(obra.get("texto_curatorial", ""))
                st.write(obra.get("enciclopedia", ""))
                st.markdown("**Ontologias associadas:**")
                st.markdown(" ".join([f"<span class='chip'>{x}</span>" for x in obra.get("ontologias", [])]), unsafe_allow_html=True)
            with st.form(f"tag_form_{obra['id']}"):
                tag = st.text_input("Adicionar tag livre")
            ...
            if not ut.empty:
                ...
                if not local.empty:
                    ...
                    st.markdown(" ".join([f"<span class='chip'>{t}</span>" for t in local['tag'].tolist()]), unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
...
def run() -> None:
    st.set_page_config(page_title=APP_TITLE, layout="wide", initial_sidebar_state="expanded", page_icon="🧠")
    bootstrap()
    init_session()
    render_shell_start()
    access_panel()
