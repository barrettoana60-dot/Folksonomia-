...
def apply_font_size():
    """Aplica o tamanho da fonte via JavaScript"""
    ...
    """
    st.components.v1.html(font_size_js, height=0)

def apply_theme():
    """Aplica o tema via CSS"""
...
def apply_reduce_motion():
    ...
    if st.session_state.accessibility['reduce_motion']:
        motion_css = """
        <style>
            * {
...
def generate_image_description(image_url):
    ...
    try:
        ...
        return f"Descrição automática não disponível para esta imagem."
...
def load_css():
    ...
    * {
    ...
    .stApp {
        background: var(--bg-gradient, linear-gradient(-45deg,#000 0%,#001F3F 25%,#000 50%,#001F3F 75%,#000 100%));
...
# ── INTERFACE PRINCIPAL ───────────────────────────────────────────────
def show_header():
    # Barra de acessibilidade com botões funcionais
    st.markdown("""
    <div class="accessibility-bar">
        <button class="accessibility-btn" onclick="document.body.style.fontSize='larger'">
            A+ Aumentar Texto
        </button>
            ...
            if st.session_state.accessibility.get('audio_enabled', False):
                ...
                with col1:
                    if st.button("🔊 Áudio", key=f"audio_{obra['id']}"):
                        text_to_speech(f"{obra['titulo']} por {obra['artista']}. {img_desc}")
                with col2:
                    st.markdown(f"<small>{img_desc[:50]}...</small>", unsafe_allow_html=True)
